...
public class PartitionDefinitionOracle  extends PartitionDefinition{
	...
	static final String FUNCTION_PARTITION_TO_DAYS = "TO_DATE";
	...
	public PartitionDefinitionOracle(){
		super();
	}
	...
	String sql = "PARTITION BY KEY ("
		+ Utility.getArrayListAsDelimitedString(
				checkReservedWord(partitionDefinition.getAttributes()), ",")
	...
	sql.append(Utility.getArrayListAsDelimitedString(checkReservedWord(pd.getAttributes()),
		","));
	if (pd.getFunction() != null && !pd.equals(""))
	    sql.append(")");
	sql.append(") \n PARTITIONS ");
	...
	if (pd.getPartitions() == null
			...
			&& pd.getEndDate() != null) {
		sql.append("\n INTERVAL(NUMTOYMINTERVAL(1,'"+ pd.getInterval()  +"'))\n");
	...
	while (calCurr.before(cal2)) {
	    ...
	    part = new Partition();
	    part.setValueDeclaration("TO_DATE('" + newDate + "','YYYY-MM-DD')");
	...
	private ArrayList checkReservedWord(ArrayList words_in) {
		...
		Iterator <String>it = words_in.iterator();
		
		while(it.hasNext()) {
			String word = it.next();
			if(words.contains(word.toUpperCase())) {
				words_in.remove(word);
				words_in.add("\"" + word + "\"");
			}
